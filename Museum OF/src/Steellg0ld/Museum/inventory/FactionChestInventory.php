...
class FactionChestInventory {
    public static function openInventory(MPlayer $player, MFaction $faction){
        Plugin::getInstance()->getAsyncDatabase()->executeSelectRaw("SELECT chest FROM chests WHERE faction = '". $faction->getIdentifier() . "'",[],function (array $row) use ($player, $faction){
            $menu = InvMenu::create(InvMenu::TYPE_CHEST);
            $menu->send($player, "Coffre de faction");
            $menu->getInventory()->setContents(unserialize(base64_decode($row[0]["chest"])));
