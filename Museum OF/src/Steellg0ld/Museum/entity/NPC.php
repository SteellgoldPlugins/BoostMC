...
abstract class NPC extends Human
{
    ...
    abstract public function onClick(MPlayer $player) : void;
    ...
    public static function createNBT(Vector3 $pos) : CompoundTag
    {
        ...
        $nbt->setTag(clone Resources::getSkinTag());
        return $nbt;
    }
    ...
    public function onUpdate(int $currentTick) : bool{
        ...
        if (!$this->isClosed()) {
            ...
            if (Plugin::getInstance()->getMicroTime() >= $this->moveUpdate) {
                $this->moveUpdate = Plugin::getInstance()->getMicroTime() + 400;
                foreach (Plugin::getInstance()->getServer()->getOnlinePlayers() as $player) {
